...
export default function ClientDashboardPage() {
  ...
  const filteredTickets = useMemo(() => {
    return myTickets.filter((t) => {
      ...
      // El historial se define SOLO por el estatus final.
      // Si el cliente evaluó negativo, el estatus será 'qa' o 'en proceso', 
      // por lo tanto NO entrará aquí y se quedará en Activos.
      const isHistory = ['cerrado', 'cancelado', 'revision_interna'].includes(status);
  ...
  const getPublicStatus = (internalStatus: string) => {
    ...
    // Mapeo de colores para el cliente
    if (['pendiente', 'sin asignar'].includes(s)) return { label: 'EN ESPERA', color: 'bg-amber-100 text-amber-700' };
    
    // Agregamos 'qa' aquí para que el cliente sepa que se está revisando su queja
    if (['in_progress', 'asignado', 'en proceso', 'qa'].includes(s)) return { label: 'EN PROCESO', color: 'bg-blue-100 text-blue-700' };
  ...
  // --- 5. ENVIAR EVALUACIÓN (LÓGICA CORREGIDA) ---
  const submitEvaluation = async () => {
    if (!evaluatingTicket) return;
    setIsSubmitting(true);

    // 🔴 Lógica de Cambio de Estatus:
    // Si se resolvió -> 'revision_interna' (Pasa a Historial)
    // Si NO se resolvió -> 'qa' (Se queda en Activos como "En Proceso")
    const nextStatus = isProblemSolved ? 'revision_interna' : 'qa'; 

    const { error } = await supabase.from('tickets').update({
        status: nextStatus,
        auditoria_cliente: isProblemSolved ? 'Solución Confirmada' : 'No resuelto - Requiere Revisión',
        satisfaction_rating: isProblemSolved ? rating : 0,
        feedback_cliente: feedback,
        evaluacion_privada: true, // Mantenemos el flag de que ya opinó, pero el filtro usa el estatus
    ...
    if (!error) {
      ...
      if (isProblemSolved) {
          alert("✅ ¡Gracias! El servicio se ha movido a tu Historial.");
      } else {
          alert("⚠️ Entendido. El servicio se mantendrá ACTIVO para revisión de calidad.");
      }
    } else {
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-6 font-sans relative overflow-hidden">
        ...
        <div className="relative z-10 w-full max-w-lg bg-white rounded-[3rem] shadow-2xl p-10 text-center animate-in zoom-in-95 duration-500 border border-slate-100">
          ...
          <h1 className="text-2xl font-black text-[#0a1e3f] uppercase tracking-tight mb-2">Cuenta en Validación</h1>
          <p className="text-slate-400 font-bold uppercase text-xs tracking-widest mb-8">Bienvenido, {currentUser.organization || 'Cliente'}</p>
          <div className="bg-slate-50 rounded-3xl p-6 text-left space-y-4 border border-slate-100 mb-8">
            ...
            <div className="flex gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500">Estamos asignándote un <b className="text-slate-700">Coordinador Especializado</b>. En cuanto se active, podrás enviar solicitudes y ver tus servicios aquí.</p>
              </div>
            </div>
          </div>
          <button onClick={handleLogout} className="w-full py-4 bg-[#0a1e3f] hover:bg-[#152c55] text-white rounded-2xl font-black text-xs uppercase tracking-widest transition-all shadow-xl active:scale-95 flex items-center justify-center gap-2">
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-4 md:p-10 font-sans text-slate-800 space-y-6 pb-24 text-left animate-in fade-in duration-500">
      ...
      <section className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-6">
        <div className="bg-gradient-to-r from-[#0a1e3f] to-[#1a3a6d] p-8 md:p-12 rounded-[2.5rem] shadow-xl w-full text-white relative overflow-hidden flex justify-between items-center">
          <div className="relative z-10 space-y-2">
            ...
            <p className="text-blue-100 text-sm font-medium opacity-90">Portal de Servicios Corporativos</p>
          </div>
      ...
      <section className="max-w-7xl mx-auto flex flex-col lg:flex-row justify-between items-center gap-4 bg-white p-4 rounded-[2rem] shadow-sm border border-slate-100">
        ...
        <div className="flex w-full lg:w-auto gap-4">
            <div className="relative flex-1 lg:w-80">
                ...
                <input type="text" placeholder="Buscar por folio..." className="w-full bg-slate-50 border border-slate-200 rounded-full py-3 pl-12 pr-4 text-xs font-bold outline-none uppercase" onChange={(e) => setSearchTerm(e.target.value)} />
            </div>
            <Link href="/solicitud" className="bg-[#00C897] hover:bg-[#00b085] text-[#0a1e3f] font-black px-6 py-3 rounded-full flex items-center gap-2 transition-all shadow-lg active:scale-95 text-[10px] tracking-widest uppercase whitespace-nowrap">
              ...
              <tbody className="divide-y divide-slate-50">
                {filteredTickets.length === 0 ? (
                  ...
                  filteredTickets.map((ticket) => {
                    ...
                    // Solo permitir evaluar si está en 'realizado' y NO ha sido evaluado exitosamente antes (o está en loop de QA)
                    // Si ya está en 'qa', significa que el cliente ya dijo que NO, así que esperamos acción del admin, no del cliente.
                    const canEvaluate = ['ejecutado', 'realizado'].includes((ticket.status || '').toLowerCase());
                      ...
                      <tr key={ticket.id} className="hover:bg-slate-50/50 transition-colors group">
                        ...
                        <td className="p-6 align-top">
                          ...
                          <p className="text-[10px] text-slate-500 flex items-center gap-1 uppercase">
                            <MapPin size={10} className="text-slate-400"/> {ticket.location || 'N/A'}
                          </p>
                        ...
                        <td className="p-6 text-center align-top">
                          <div className="flex justify-center gap-2">
                            <button onClick={() => setViewingDetails(ticket)} className="p-2 bg-slate-50 text-slate-600 rounded-xl hover:bg-[#0a1e3f] hover:text-white transition-all tooltip"><Eye size={16} /></button>
                            {canEvaluate && (
                                <button onClick={() => setEvaluatingTicket(ticket)} className="p-2 bg-purple-50 text-purple-600 rounded-xl hover:bg-purple-600 hover:text-white transition-all animate-pulse"><Star size={16} /></button>
                            )}
                          </div>
                        </td>
                      </tr>
      ...
      {/* MODALES (DETALLE / EVALUACIÓN) */}
      {viewingDetails && (
        <div className="fixed inset-0 bg-[#0a1e3f]/80 backdrop-blur-md z-[100] flex items-center justify-center p-4 animate-in fade-in">
          <div className="bg-white rounded-[2.5rem] w-full max-w-2xl overflow-hidden shadow-2xl flex flex-col animate-in zoom-in-95 duration-300">
            <div className="p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
              <div>
                <h3 className="text-lg font-black text-[#0a1e3f] uppercase tracking-tight flex items-center gap-2"><Info className="text-blue-600" size={20}/> Detalle del Servicio</h3>
            ...
            <div className="p-8 space-y-6 max-h-[70vh] overflow-y-auto custom-scrollbar">
                ...
                {viewingDetails.technical_result && (
                    <div className="bg-emerald-50/50 p-5 rounded-3xl border border-emerald-100">
                        <p className="text-[10px] font-black text-emerald-600 uppercase tracking-widest mb-2 flex items-center gap-2"><CheckCircle size={14}/> Reporte Técnico</p>
      ...
      {evaluatingTicket && (
        <div className="fixed inset-0 bg-[#0a1e3f]/90 backdrop-blur-sm z-[100] flex items-center justify-center p-4 animate-in fade-in">
          <div className="bg-white rounded-[3rem] w-full max-w-lg p-10 relative animate-in zoom-in-95 duration-300 shadow-2xl">
            <button onClick={() => setEvaluatingTicket(null)} className="absolute right-8 top-8 text-slate-300 hover:text-slate-500 transition-colors"><X /></button>
            <div className="text-center space-y-4">
              <div className="w-16 h-16 bg-purple-50 text-purple-600 rounded-full flex items-center justify-center mx-auto mb-6 shadow-sm"><Star size={32} className="fill-purple-600"/></div>
              <h3 className="text-2xl font-black uppercase text-[#0a1e3f] tracking-tight">Tu Opinión Importa</h3>
              <p className="text-sm text-slate-500 font-medium">Ayúdanos a mejorar evaluando el servicio <span className="font-black text-purple-600">{evaluatingTicket.codigo_servicio}</span></p>
            </div>
            <div className="mt-8 space-y-6">
              ...
              {isProblemSolved !== null && (
                  <div className="animate-in fade-in slide-in-from-bottom-4 space-y-4">
                    <textarea value={feedback} onChange={(e) => setFeedback(e.target.value)} className="w-full bg-slate-50 border-2 border-slate-100 rounded-3xl p-5 text-xs font-medium outline-none focus:border-purple-200 h-24 resize-none transition-all" placeholder={isProblemSolved ? "¿Qué te pareció la atención?" : "Por favor cuéntanos qué faltó..."} />
