...
import RolesManager from '@/components/dashboard/super-admin/RolesManager'; // 👈 IMPORTANTE

import { Search, RefreshCw, LayoutGrid, LogOut, Loader2, Settings } from 'lucide-react';

// Agregamos 'configuracion' al tipo de Tabs
...
export default function DashboardPage() {
  ...
  // TABS VISIBLES (Ya no ponemos Roles aquí, irá en el engrane)
    ...
    <div className="min-h-screen bg-slate-50 font-sans text-slate-800">
      ...
      <header className="bg-[#0a1e3f] text-white p-6 md:px-10 md:py-8 flex justify-between items-center shadow-2xl sticky top-0 z-30">
        ...
        <div className="flex items-center gap-6">
          ...
          {/* ⚙️ BOTÓN DE CONFIGURACIÓN (SOLO SUPERADMIN) */}
          ...
          <button onClick={handleLogout} className="bg-white/10 hover:bg-red-500/20 hover:text-red-400 p-3 rounded-xl transition-all border border-white/10">
            <LogOut size={20} />
          </button>
        </div>
      </header>
      ...
      <div className="p-6 md:p-10 pb-24 max-w-[1900px] mx-auto">
        ...
        {/* TABS (Si NO estamos en configuración, mostramos las tabs normales) */}
        ...
        {/* SI ESTAMOS EN CONFIGURACIÓN (Mostramos botón volver) */}
        {activeTab === 'configuracion' && (
             <div className="mb-8">
                 <button onClick={() => setActiveTab('operaciones')} className="text-slate-400 hover:text-[#0a1e3f] font-black text-xs uppercase flex items-center gap-2 transition-colors">
                     ← Volver al Panel
                 </button>
             </div>
        )}
        ...
        {/* ⚙️ VISTA DE CONFIGURACIÓN DE ROLES */}
